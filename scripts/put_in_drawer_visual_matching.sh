...
declare -a env_names=(
  ...
  # PlaceIntoClosedMiddleDrawerCustomInScene-v0 # cogact  not evaluated
  # PlaceIntoClosedBottomDrawerCustomInScene-v0 # cogact  not evaluated
)
...
for urdf_version in "${urdf_version_arr[@]}"; do
  ...
  EXTRA_ARGS="--enable-raytracing True --additional-env-build-kwargs station_name=mk_station_recolor light_mode=simple disable_bad_material=True urdf_version=${urdf_version} model_ids=baked_apple_v2"
  ...
  EvalOverlay() {
    ...
    CUDA_VISIBLE_DEVICES=${gpu_id} python simpler_env/main_inference.py --policy-model ${policy_model} --ckpt-path ${ckpt_path} --action-ensemble-temp ${action_ensemble_temp} --logging-dir ${logging_dir} \
      ...
      ${EXTRA_ARGS} --ensembler ${ensembler}
    echo "Done A0"
    # B0
    CUDA_VISIBLE_DEVICES=${gpu_id} python simpler_env/main_inference.py --policy-model ${policy_model} --ckpt-path ${ckpt_path} --action-ensemble-temp ${action_ensemble_temp} --logging-dir ${logging_dir} \
      --robot google_robot_static \
