...
def run_maniskill2_eval_single_episode(
    ...
    print(f"[DEBUG] Enter run_maniskill2_eval_single_episode, obj_episode_id={obj_episode_id}")
    ensembler = getattr(model, "action_ensembler", None)
    ...
    for robot_init_x in args.robot_init_xs:
        for robot_init_y in args.robot_init_ys:
            for robot_init_quat in args.robot_init_quats:
                ...
                if args.obj_variation_mode == "xy": #默认是 xy 
                ...
                elif args.obj_variation_mode == "episode": # bridge是用episode这个分支. 
                    for obj_episode_id in range(
                        args.obj_episode_range[0], args.obj_episode_range[1]
                    ): #跑60次, 所以很久. 
                        # import ipdb; ipdb.set_trace()
                        print(args.obj_episode_range[0], args.obj_episode_range[1])
                        print(f"Running episode {obj_episode_id}")
                        success_arr.append(
                                run_maniskill2_eval_single_episode(
