...
from model.svla_utils import get_svla
from train.monkey_patch import replace_forward, replace_predict_action
...
class SpatialVLAInference:
    def __init__(
        ...
        if policy_setup == "widowx_bridge":
            ...
            action_ensemble = True
            self.sticky_gripper_num_repeat = 10 #设成10 , 和cogact一样, 这就是googlerobot的成功秘诀!
        else:
        ...
        self.obs_horizon = (self.processor.num_obs_steps -
                            1) * self.processor.obs_delta + 1
    ...
    def step(
        ...
        inputs = self.processor(
            images=images, text=prompt, unnorm_key=self.unnorm_key, return_tensors="pt", do_normalize=False)
        with torch.no_grad():
    ...
    def visualize_epoch(
        ...
        pred_actions = np.array(
            [
                np.concatenate(
                    [a["world_vector"], a["rotation_delta"],
                        a["open_gripper"]], axis=-1
                )
...
class SpatialVLALisaInference(SpatialVLAInference):
    def __init__(
        self,
        saved_model_path: str = "IPEC-COMMUNITY/spatialvla-4b-224-pt",
        unnorm_key: Optional[str] = None,
        ...
        **kwargs,
        ...
        if policy_setup == "widowx_bridge":
            ...
            action_ensemble = True
            self.sticky_gripper_num_repeat = 1
            base_vla_path = "IPEC-COMMUNITY/spatialvla-4b-224-sft-bridge"
        elif policy_setup == "google_robot":
            unnorm_key = (
                "fractal20220817_data/0.1.0" if unnorm_key is None else unnorm_key
            )
            base_vla_path = "IPEC-COMMUNITY/spatialvla-4b-224-sft-fractal"
            action_ensemble = True
        ...
        self.vla = get_svla(base_vla_path, saved_model_path)
        # tokenizer =self.processor.tokenizer
        # self.vla.resize_token_embeddings(len(tokenizer)) # 讲道理这应该是训练就保存了,  # otherwise get_input_embeddings() will cuda error

        replace_forward(self.vla)
        replace_predict_action(self.vla)
    ...
    def step(
        ...
        with torch.no_grad():
            if hasattr(self.processor, "action_tokenizer"):
                ...
                # raw_actions = self.vla.predict_action(inputs)["actions"]
                # raw_actions = raw_actions.cpu().numpy()
                hidden_states = self.vla.lisa_predict_action(inputs)
                hidden_states = hidden_states.float()
                raw_actions = self.vla.action_head.predict_action(
                    hidden_states)
                raw_actions = raw_actions.cpu().numpy()
                raw_actions = raw_actions[0]  # bs = 1
